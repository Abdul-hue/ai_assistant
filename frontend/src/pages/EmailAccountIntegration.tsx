    ...
    <AppLayout>
      <div className="w-full">
        ...
        {loading ? (
          ...
          <div className="space-y-6">
            {/* Quick Stats Bar */}
            <div className="bg-gradient-to-r from-primary/10 to-whatsapp-teal/10 dark:from-primary/20 dark:to-whatsapp-teal/20 rounded-xl p-6 border border-primary/20 dark:border-primary/30">
              <div className="flex items-center justify-between flex-wrap gap-4">
                <div className="flex items-center gap-8">
                  <div>
                    <p className="text-sm font-semibold text-muted-foreground mb-1">Total Accounts</p>
                    <p className="text-3xl font-bold text-foreground">{imapAccounts.length}</p>
                  </div>
                  <div>
                  ...
                  {imapAccounts.filter(acc => acc.needs_reconnection).length > 0 && (
                    <div>
                      <p className="text-sm font-semibold text-muted-foreground mb-1">Needs Attention</p>
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-2">
              {imapAccounts.map((account, index) => (
                <Card 
                  key={account.id} 
                  className="group hover:shadow-lg transition-all duration-200 border-2 border-border bg-card hover:border-primary dark:hover:border-primary"
                ...
                >
                  <CardContent className="p-6">
                    ...
                    <div className="flex items-start justify-between mb-4">
                      <div className="flex items-center gap-3 flex-1 min-w-0">
                        ...
                        <div className="relative">
                          <div className="h-12 w-12 rounded-full bg-gradient-primary flex items-center justify-center text-white font-semibold text-lg shadow-lg">
                            {account.email.charAt(0).toUpperCase()}
                          </div>
                          {/* Active status dot */}
                          {account.is_active && !account.needs_reconnection && (
                            <div className="absolute -bottom-1 -right-1 h-4 w-4 bg-primary rounded-full border-2 border-white dark:border-gray-900 shadow-sm" />
                          )}
                          {account.needs_reconnection && (
                            <div className="absolute -bottom-1 -right-1 h-4 w-4 bg-warning rounded-full border-2 border-white dark:border-gray-900 shadow-sm" />
                          )}
                        </div>
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-2 mt-2">
                            ...
                            <Badge className="bg-primary/10 dark:bg-primary/20 text-primary dark:text-primary border border-primary/30 dark:border-primary/40 text-xs font-semibold px-2.5 py-1">
                    ...
                    <div className="flex items-center gap-2">
                      <Button 
                        className="flex-1 bg-gradient-primary hover:opacity-90 text-white font-semibold text-base h-11 shadow-lg" 
                      ...
                      <Button 
                        variant="outline" 
                        size="icon"
                        className="border-2 border-border hover:border-primary hover:bg-primary/10 text-foreground hover:text-primary"
                        onClick={() => navigate(`/emails/${account.id}`)}
                        title="View Settings"
                      >
            ...
            <Button 
              variant="outline" 
              className="w-full h-20 border-dashed border-2 border-border hover:border-primary hover:bg-primary/10 transition-all duration-200 rounded-xl text-base font-semibold text-foreground hover:text-primary"
            ...
            <Alert className="bg-primary/10 dark:bg-primary/20 border-primary/30 dark:border-primary/40">
              <Info className="h-5 w-5 text-primary dark:text-primary flex-shrink-0" />
              <AlertTitle className="text-base font-semibold text-foreground">
                Need help connecting your email?
              </AlertTitle>
              <AlertDescription className="text-sm font-medium text-muted-foreground mt-1">
          ...
          <div className="flex flex-col items-center justify-center py-20 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="relative mb-8">
              <div className="h-32 w-32 rounded-full bg-gradient-primary/20 dark:bg-gradient-primary/30 flex items-center justify-center shadow-xl">
                <Mail className="h-16 w-16 text-primary dark:text-primary" />
              </div>
              <div className="absolute -bottom-2 -right-2 h-12 w-12 rounded-full bg-card shadow-lg flex items-center justify-center border-2 border-primary/30 dark:border-primary/40">
                <Plus className="h-6 w-6 text-primary dark:text-primary" />
              </div>
            </div>

            {/* Text Content */}
            <h2 className="text-2xl font-bold mb-3 text-foreground">No Email Accounts Connected</h2>
            <p className="text-muted-foreground text-center max-w-md mb-8 text-base font-medium">
            ...
            <Button 
              size="lg" 
              className="bg-gradient-primary hover:opacity-90 text-white shadow-lg hover:shadow-xl h-12 px-8 text-base font-semibold transition-all duration-200 hover:scale-105"
              onClick={() => navigate("/email-integration/imap-smtp/connect")}
            >
              <Plus className="h-5 w-5 mr-2" />
            ...
            <div className="mt-12 grid grid-cols-3 gap-8 text-center max-w-2xl">
              <div>
                <div className="h-12 w-12 rounded-full bg-primary/10 dark:bg-primary/20 flex items-center justify-center mx-auto mb-2">
                  <Shield className="h-6 w-6 text-primary dark:text-primary" />
                </div>
                <p className="text-sm font-semibold text-foreground mb-1">Secure</p>
                <p className="text-xs text-muted-foreground font-medium">End-to-end encrypted</p>
              </div>
              <div>
                <div className="h-12 w-12 rounded-full bg-primary/10 dark:bg-primary/20 flex items-center justify-center mx-auto mb-2">
                  <Zap className="h-6 w-6 text-primary dark:text-primary" />
                </div>
                <p className="text-sm font-semibold text-foreground mb-1">Fast</p>
                <p className="text-xs text-muted-foreground font-medium">Real-time sync</p>
              </div>
              <div>
                <div className="h-12 w-12 rounded-full bg-primary/10 dark:bg-primary/20 flex items-center justify-center mx-auto mb-2">
                  <Check className="h-6 w-6 text-primary dark:text-primary" />
                </div>
                <p className="text-sm font-semibold text-foreground mb-1">Easy</p>
                <p className="text-xs text-muted-foreground font-medium">Setup in minutes</p>
              </div>
            </div>
          </div>
        )}
