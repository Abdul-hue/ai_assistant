...
try {
    ...
    if ($connections) {
        $processIds = $connections | Select-Object -ExpandProperty OwningProcess -Unique | Where-Object { $_ -ne 0 -and $_ -gt 0 }
        ...
        if ($processIds) {
            ...
            Write-Host "Found connections but no valid processes (only system processes like PID 0)" -ForegroundColor Yellow
            Write-Host "Port may appear in use but no user processes found. Trying to start server anyway..." -ForegroundColor Cyan
        }
